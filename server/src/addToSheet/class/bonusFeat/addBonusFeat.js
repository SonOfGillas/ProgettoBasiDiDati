...
exports.handler = async (event) => {
	...
	try {
		response = new Promise((resolve) => {
			...
			const queryAddBonusFeat ="INSERT INTO TalentiBonus (CodPer,NomeClasse,NomeTalento) VALUES ("+event['CodPer']+",'"+event['NomeClasse']+"','"+event['NomeTalento']+"')";
					...
					db.query(queryPgFeat, (err, pgFeat) => {
						...
						db.query(queryPgBonusFeat, (err, bonusFeat) => {
								...
								db.query(queryRequiredFeat, (err, requiredFeat) => {
									...
									if(pgHaveAllRequiredFeats){
										db.query(queryAddBonusFeat, (err, addFeat) => {
											if (err) throw err;
												resolve({
													statusCode: 202,
													body: addFeat
