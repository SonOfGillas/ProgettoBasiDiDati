...
exports.handler = async (event) => {
	...
	try {
		response = new Promise((resolve) => {
			...
			const querySetClassLevel =
				'update Classi set Livello=' +
				event['Livello'] +
				' WHERE CodPer =' +
				event['CodPer'] +
				" AND NomeClasse='" +
			...
			from (Select C.Livello,DadoVita*Livello as PF,floor(BAB*Livello) as BAB,floor(Tempra*Livello) as Tempra,floor(Volonta*Livello) as Volonta,floor(Riflessi*Livello) as Riflessi,CompArmature,CompScudi,CompArmiEsotiche,CompArmiGuerra, Incantatore
				...
				where C.NomeClasse=DC.NomeClasse And CodPer=` +
				event['CodPer'] +
				`) stat;`;
			const queryGetRaceDimension =
				'select Taglia from Razze where Nome=(select razza from Personaggi where CodPer=' + event['CodPer'] + ')';
			const queryGetPgStat =
				'Select Forza,Destrezza,Costituzione,Saggezza from Personaggi where CodPer=' + event['CodPer'];

			db.connect();

			db.query(querySetClassLevel, (err, resultSetClassLevel) => {
				if (err) throw err;
				db.query(queryGetCLassStats, (err, classStat) => {
					...
					db.query(queryGetRaceDimension, (err, raceDimension) => {
						...
						db.query(queryGetPgStat, (err, pgStat) => {
							...
							const queryUpdatePersonaggio =
								'update Personaggi set HP=' +
								PF +
								', BAB=' +
								classStat[0].BAB +
								', BMC=' +
								...
								Volonta +
								', CompArmature=' +
								classStat[0].CompArmature +
								', CompScudi=' +
								classStat[0].CompScudi +
								', CompeArmiEsotiche=' +
								classStat[0].CompArmiEsotiche +
								', CompeArmiGuerra=' +
								classStat[0].CompArmiGuerra +
								', Incantatore=' +
								classStat[0].Incantatore +
								' where CodPer=' +
								event['CodPer'];
							...
							db.query(queryUpdatePersonaggio, (err, result) => {
								...
								resolve({
									statusCode: 202,
									body: {
										setClassLevel: resultSetClassLevel,
